...
const DraggableBoxes2 = () => {
    ...
    const [boxes, setBoxes] = useState([
        { id: 'Start', defaultPosition: { x: window.innerWidth / 2 - 100, y: window.innerHeight / 2 }, name: 'Start' },
        { id: 'End', defaultPosition: { x: window.innerWidth / 2 + 100, y: window.innerHeight / 2 }, name: 'End' },
    ]);
    ...
    useEffect(() => {
        const fetchAvailableBoxes = async () => {
            if (userEmail) {
                try {
                    const response = await fetch(`http://localhost:3000/box/getBoxes/${userEmail}`, {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                    ...
                    if (data.message === "Boxes fetched successfully!" && Array.isArray(data.boxes)) {
    ...
    const handleAddBoxes = (selectedBoxes) => {
        const newBoxes = selectedBoxes.map((box) => {
            ...
            // Set specific positions for Start and End boxes
            if (box.title === "Start") {
                position = { x: window.innerWidth / 2 - 100, y: window.innerHeight / 2 }; // Center-left
            } else if (box.title === "End") {
                position = { x: window.innerWidth / 2 + 100, y: window.innerHeight / 2 }; // Center-right
            }
    ...
    return (
        <>
            ...
            <div className="w-full flex items-center justify-center gap-2 mt-[60px]">
                <button className="bg-red-600 px-3 py-2 text-white m-4 rounded-md" onClick={handleDelete}>
                    Delete
                </button>
                <button
                    className="bg-blue-600 px-3 py-2 text-white m-4 rounded-md"
            ...
            <Xwrapper>
                {boxes.map((box) => (
                    <Box
                        key={box.id}
                        id={box.id}
                        style={box.defaultPosition}
                        onDoubleClick={handleDoubleClick}
                        isSelected={selectedBoxId === box.id} // Highlight if selected
                    />
                ))}
